 ...
 *
 * @url https://github.com/paquettg/php-html-parser
 */

namespace HTMLParser;

use \HTMLParser\Dom\AbstractNode;
use \HTMLParser\Dom\HtmlNode;
use \HTMLParser\Dom\TextNode;
use \HTMLParser\Exceptions\NotLoadedException;
use \HTMLParser\Exceptions\StrictException;
...
class Dom {
   ...
   * @return $this
   */
  public function loadStr($str, $options = []) {
    $this->options = new Options;
    $this->options->setOptions($this->globalOptions)->setOptions($options);
   ...
   *
   * @return \HTMLParser\Dom\AbstractNode
   */
  ...
  protected function clean($str) {
    ...
    if ($this->options->get('removeScripts') == true) {
      $str = preg_replace('/<\s*script[^>]*[^\/]>(.*?)<\s*\/\s*script\s*>/', '', $str);
      $str = preg_replace('/<\s*script\s*>(.*?)<\s*\/\s*script\s*>/', '', $str);
    }
