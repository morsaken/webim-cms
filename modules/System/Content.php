...
class Content extends Controller {
        ...
        case 'category':
          ...
          if (count($ids) || count($urls)) {
            $this->whereExists(function ($exists) use ($ids, $urls) {
              ...
              if (count($urls)) {
                $exists->where(function($where) use ($urls) {
                  $where->whereIn('cat.name', $urls);
                  $where->orWhereIn('cat.url', $urls);
                });
              }
   ...
   * @return array
   */
  protected function options($rows, $params = array()) {
    if (in_array(__FUNCTION__, $this->called)) {
      return $rows;
    }

    if (!in_array('meta', $this->called)) {
      $this->with('meta');
    }
    ...
    return array_map(function (&$row) {
      try {
        $row->options = new \stdClass();

        if (isset($row->meta->options)) {
          $options = @json_decode($row->meta->options);
  ...
  protected function formGroups($rows, $params = array()) {
    ...
    $query = DB::table('sys_content_form_value as fv')
      ...
      ->join('sys_form_group as fg', 'fg.id', 'fp.group_id')
      ->join('sys_form_field as ff', 'ff.id', 'fp.field_id')
      ->whereIn('fv.content_id', $this->ids())->orderBy('fp.order')->get(array(
        'fv.content_id',
        'fg.id as group_id',
        'fg.label as group_label',
        'ff.id',
        ...
        if (isset($row->children) && count($row->children)) {
          foreach ($row->children as $child) {
            ...
            if (isset($child->children) && count($child->children)) {
              $deep += forward_static_call(array('static', __FUNCTION__), $child->children, $child->id, ($level + 2));
            }
          }
        }
