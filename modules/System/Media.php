...
class Media extends Content {
  ...
  protected function files($rows, $params = array()) {
    ...
    return array_map(function (&$row) use ($rows, $posters, $params) {
      ...
      if ($default instanceof File && $default->isFile()) {
        $defaultImage = Picture::file($default);

        if ($width || $height) {
          $defaultImage = $defaultImage->size($width, $height);
        }
      } else {
