...
class Circular {
  ...
  public static function register(Manager $manager) {
    $manager->addRoute($manager->prefix . '/circular', __CLASS__ . '::getIndex');
    $manager->addRoute($manager->prefix . '/circular/form/?:id', __CLASS__ . '::getForm');
    $manager->addRoute($manager->prefix . '/circular/form/?:id', __CLASS__ . '::postForm', 'POST');
    $manager->addRoute($manager->prefix . '/circular/form/:id+', __CLASS__ . '::deleteForm', 'DELETE');

    $manager->addMenu(lang('admin.menu.modules', 'Modüller'), $manager->prefix . '/circular', lang('admin.menu.circular', 'Sirküler'), null, 'fa fa-book');
  ...
  public function getIndex($params = array()) {
    ...
    $nav->url = url($manager->prefix . '/circular/form');
    $nav->icon = 'fa-plus';
    ...
    $manager->set('caption', lang('admin.menu.circular', 'Sirküler'));
    $manager->breadcrumb($manager->prefix . '/circular', lang('admin.menu.circular', 'Sirküler'));

    $manager->put('categories',
      Content::init()
        ->where('type', 'category')
        ->only('parent.name', 'circular')
        ->load()
        ->getListIndented('&nbsp;&nbsp;')
    );
    ...
    return View::create('modules.others.circular.list')->data($manager::data())->render();
  }
  ...
  public function getForm($params = array()) {
    ...
    $manager->set('caption', lang('admin.menu.circular', 'Sirküler'));
    $manager->breadcrumb($manager->prefix . '/circular', lang('admin.menu.circular', 'Sirküler'));
    $manager->breadcrumb($manager->prefix . '/circular/form', lang('admin.menu.' . $action, $actionTitle));

    return View::create('modules.others.circular.form')->data($manager::data())->render();
  }
