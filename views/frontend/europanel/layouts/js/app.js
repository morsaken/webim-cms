...
var App = function () {
  return {
    ...
    'plugins': function() {
      ...
      if (!$('ul.breadcrumb.fixed').length) {
        $('ul.breadcrumb').clone(true).addClass('fixed').appendTo('#content');
      }

      var breadcrumbOffset = $('ul.breadcrumb').height() * 2;
      ...
      $('#area').on('keypress keyup', function() {
        var $val = $(this).val();
        var $var = $(this).data('value');

        if (!isNaN($val) && !isNaN($var)) {
          $('#panel').text(Math.ceil($val / $var));
        }
      });
      ...
      $('.info a.close').click(function(e) {
        ...
        if (!$(this).closest('.info').hasClass('closed')) {
          $(this).closest('.info').addClass('closed');
        }
      });

      $('.info a.open').click(function(e) {
        e.preventDefault();

        if ($(this).closest('.info').hasClass('closed')) {
          $(this).closest('.info').removeClass('closed');
          App.divScroller();
        }
      });
      ...
      if ($().iLightBox) {
        ...
        $('.gallery .media', '#content').iLightBox().destroy();
        $('.poster', '#content').each(function() {
          $(this).iLightBox();
        });
        $('.gallery .media', '#content').iLightBox();
      }
      ...
      if (window.history.pushState) {
        $('a.pop', context).click(function (e) {
          ...
          if (url.length && url !== '#' && url !== cur && !$(this).prop('target')) {
            ...
            var page_title = $('title').data('title');

            if (title && title.length && title !== page_title) {
        ...
        window.onpopstate = function (e) {
          App.setPage(e.state ? e.state.url : null);
        };
      }
    },
    ...
    'divScroller': function() {
      $('.scroller', '#content').each(function() {
        $(this).css('height', (
          ...
          - $(this).parent().find('form').height() - 100)
        );
      }).niceScroll({
        cursorcolor: '#FB3370',
        cursorborder: 'none'
    ...
    'forms': function(form) {
      ...
      $(form).submit(function(e) {
        ...
        $.ajax({
          ...
          $.notify(result.text, result.success ? 'success' : 'warn');
