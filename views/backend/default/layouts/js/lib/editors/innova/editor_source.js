...
function InnovaEditorUtil()
  ...
  for(var i=0;i<oScripts.length;i++)
    ...
    if(sSrc.indexOf("innova/editor.js")!=-1) this.scriptPath=oScripts[i].src.replace(/editor.js/ig,"");
    }
...
function InnovaEditor(oName)
  ...
  for(var i=0;i<oScripts.length;i++)
    {
    var sSrc=oScripts[i].src.toLowerCase();
    if(sSrc.indexOf("innova/editor.js")!=-1) this.scriptPath=oScripts[i].src.replace(/editor.js/,"");
    }
...
function applyFontName(val)
  ...
  var oEditor=eval("idContent"+this.oName);
  var oSel=oEditor.document.selection.createRange();
  this.hide();//ini menyebabkan text yg ter-select menjadi tdk ter-select di framed-pages.
